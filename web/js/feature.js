...
$("#viewer").on("show.bs.modal", (event) => {
  ...
  let obj =
    ...
    file +
    "' id='review' style='width: 100%; height: 100%;'></object>";
  $(".modal-body").append(obj);

  let picture_book_name = button.getAttribute("id");
  let picture_book_name_cn = button.getAttribute("data-book-name-cn");

  $("#title_en").text(picture_book_name);
  $("#title_cn").text(picture_book_name_cn);

  // 搜索时
  if (window.location.href.includes("search")) {
    let search = window.location.href.match(/search=(.+)/)[1];
    history.pushState(
      "",
      "",
      `index.html?search=${search}&name=${picture_book_name}`
    );
  }
  // 翻页阅读时
    ...
    history.pushState(
      "",
      "",
      `index.html?page=${page}&name=${picture_book_name}`
    );
  }
});
$("#viewer").on("hide.bs.modal", () => {
  ...
  if (window.location.href.includes("search")) {
    let search = window.location.href.match(/search=(.+)/)[1];
    history.pushState("", "", `index.html?search=${search}`);
  }
  // 翻页阅读时
  else if (window.location.href.includes("page")) {
    let page = window.location.href.match(/page=(\d+)/)[1];
...
function search() {
  ...
  window.open(
    window.location.href.replace(
      /\/[(index)|(page)].+/,
      `/page/search.html?search=${keyword}`
    )
  );
}
...
function setPagination(current_page, page_len) {
  let page_first =
    '<li class="page-item" title="首页"><a class="page-link" aria-label="首页"><i class="bi bi-chevron-double-left"></i></a></li>';
  $("ul.pagination").append(page_first);
  let page_prev =
    '<li class="page-item" title="上一页"><a class="page-link" aria-label="上一页"><i class="bi bi-chevron-left"></i></a></li>';
  ...
  if (page_len >= 5) {
    if (current_page - 2 < 1) {
      ...
      pages = [
        current_page - 2 + offset,
        current_page - 1 + offset,
        parseInt(current_page) + offset,
        parseInt(current_page) + 1 + offset,
        parseInt(current_page) + 2 + offset,
      ];
  ...
  let page_next =
    '<li class="page-item" title="下一页"><a class="page-link" aria-label="下一页"><i class="bi bi-chevron-right"></i></a></li>';
  $("ul.pagination").append(page_next);
  let page_last =
    '<li class="page-item" title="末页"><a class="page-link" aria-label="末页"><i class="bi bi-chevron-double-right"></i></a></li>';
  ...
  $("li.page-item").on("click", (event) => {
    ...
    if (page_click_innerHTML.match(/\d/)) {
      window.location =
        ...
        "?page=" +
        page_click_innerHTML.match(/\d/);
    } else if (
      page_click_innerHTML.includes("末页") &&
      current_page != page_len
    ) {
      window.location =
        window.location.origin + window.location.pathname + "?page=" + page_len;
    } else if (page_click_innerHTML.includes("首页") && current_page != 1) {
      window.location =
        window.location.origin + window.location.pathname + "?page=1";
    } else if (page_click_innerHTML.includes("上一页") && current_page > 1) {
      window.location =
        window.location.origin +
        window.location.pathname +
        "?page=" +
        parseInt(parseInt(current_page) - 1);
    } else if (
      page_click_innerHTML.includes("下一页") &&
      current_page < page_len
    ) {
  ...
  for (let j = 0; j < 5; j++) {
    ...
    this_carousel
      ...
      .attr({
        ...
        id: book_names[num],
        "data-book-name-cn": this_book.book_cn,
    ...
    this_carousel
      .find("a")
      .eq(0)
      .attr("href", "../asset/" + this_book.file);
    this_carousel
      .find("a")
      .eq(0)
      .attr("download", "../asset/" + this_book.file);
  }
}
...
function showCards(book_info, book_num) {
  ...
  for (
    let i = (current_page - 1) * 12, j = 0;
    i < Math.min(book_num, current_page * 12);
    i++, j++
  ) {
...
function showPage(book_info) {
  ...
  $("#total").html(
    `已翻译 ${book_num} 本绘本 🎉 <br/> 如果你喜欢的话，请到 <a href="https://github.com/forestlet/bookdash-cn" class="alert-link">forestlet/bookdash-cn</a> ⭐ 吧~`
  );
...
$("#share").click(() => {
  ...
  let name = $("#title_en").text();
  $("#copy_url").text(
    `${window.location.origin}${window.location.pathname}?page=${page}&name=${name}`
  );
